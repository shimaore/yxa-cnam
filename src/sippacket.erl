...
parse_headers(Bin, Offset, Res) ->
	...
	_ ->
	    {error, no_header_body_separator_found}
    end.
...
parse_one_header(Bin, Offset) ->
    case parse_one_header_key(Bin, Offset) of
        {ok, KeyLength, ColonOffset} ->
            Key = #ptr{offset=Offset, length=KeyLength},
            case parse_one_header_values(Bin, ColonOffset + 1) of
                {ok, Comma, ValuePtrs, NextLineOffset} ->
                    This = #header{key=Key, valueptrs=ValuePtrs, comma=Comma},
                    {ok, This, NextLineOffset};
                {error, no_end_of_header, Comma, ValuePtrs, NextLineOffset} ->
                    This = #header{key=Key, valueptrs=ValuePtrs, comma=Comma},
                    {error, no_end_of_header, This, NextLineOffset};
                Error ->
                    Error
                end;
        Error ->
...
parse_one_header_key2(Bin, Offset, KeyLen, RequireColon) ->
	...
	<<_:Offset/binary, N, _/binary>> when N == ?SP; N == ?HTAB ->
		...
		false ->
		    ...
		    {error, whitespace_where_header_name_was_expected};
		true ->
...
parse_one_header_values2(Bin, Comma, Offset, StartOffset, Len, Res) ->
	...
	_ ->
	    This = #ptr{offset=StartOffset, length=Len},
	    {error, no_end_of_header, Comma, lists:reverse([This | Res]), Offset}
    end.
