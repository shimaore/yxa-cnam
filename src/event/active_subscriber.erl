...
start(PackageM, PackageS, Presenter, RequestURI, From, To, Dst, SInterval, Accept, ExtraH,
    EventSuffix, LogTag, Parent, Presentity) ->
  start(PackageM, PackageS, Presenter, RequestURI, From, To, Dst,
	  SInterval, Accept, ExtraH, EventSuffix, LogTag, Parent, Presentity, <<>>).
...
init({State1, From, To, Dst}) ->
	...
	start_generate_request(State#state.uri, From1, To1, ExtraHeaders,State#state.body),
...
handle_cast({received_notify, FromPid, Request, YxaCtx, THandler, Ctx}, State)
  when is_pid(FromPid), is_record(Request, request) ->
	...
	PackageM_Res ->
	    logger:log(debug, "Active subscriber: Extra debug: Result of ~p:request/4 was : ~p",
		       [PackageM, PackageM_Res]),
		%%SA Send the response to our parent.
		State#state.parent ! PackageM_Res
    end,
...
%% @end
%%--------------------------------------------------------------------
start_generate_request(URI, From, To, ExtraHeaders, Body) when is_record(URI, sipurl), is_record(From, contact),
							 is_record(To, contact), is_list(ExtraHeaders), is_binary(Body) ->
    Method = "SUBSCRIBE",
